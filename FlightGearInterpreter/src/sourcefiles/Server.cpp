...
Server::~Server()
{
    if (flightGearThread.joinable()) {
        flightGearThread.detach();
    }
    if (serverDatabaseThread.joinable()) {
        serverDatabaseThread.detach();
    }   
}
...
std::thread &Server::getFlightGearThread()
{
    return flightGearThread;
}

std::thread &Server::getServerDataBaseThread()
{
    return serverDatabaseThread;
}
